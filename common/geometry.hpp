...
void quad4(Model &model, int a, int b, int c, int d, int face){
	//TODO: Modify texture coordinate according to task.bmp file
	model.add_vertex(vertices[a]);
	model.add_vertex(vertices[b]);
	model.add_vertex(vertices[c]);
	int tmp = face - GL_TEXTURE_CUBE_MAP_POSITIVE_X;
	model.add_texcoord(glm::vec2(face / 6.0, 0.0));
	model.add_texcoord(glm::vec2(face / 6.0, 1.0));
	model.add_texcoord(glm::vec2((face + 1) / 6.0, 1.0));
	...
	model.add_texcoord(glm::vec2(face / 6.0, 0.0));
	model.add_texcoord(glm::vec2((face + 1) / 6.0, 1.0));
	model.add_texcoord(glm::vec2((face + 1) / 6.0, 0.0));
...
void init_texture_cube(Model &model){
	...
	quad4(model, 2, 3, 7, 6, 0); // +x
	quad4(model, 5, 4, 0, 1, 1); // -x
	quad4(model, 6, 5, 1, 2, 2); // +y
	quad4(model, 3, 0, 4, 7, 3); // -y
	quad4(model, 1, 0, 3, 2, 4); // +z
	quad4(model, 4, 5, 6, 7, 5); // -z
}
